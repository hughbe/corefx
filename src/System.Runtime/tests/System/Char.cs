...
namespace System.Runtime.Tests
{
    public static class CharTests
    {
        ...
        {
            IComparable iComparable = 'h';
            Assert.Throws<ArgumentException>(() => iComparable.CompareTo("H")); // Value not a char
        }
        ...
        {
            Assert.Throws<ArgumentOutOfRangeException>(() => char.ConvertFromUtf32(0xD800));
            Assert.Throws<ArgumentOutOfRangeException>(() => char.ConvertFromUtf32(0xDC00));
            Assert.Throws<ArgumentOutOfRangeException>(() => char.ConvertFromUtf32(0xDFFF));
            Assert.Throws<ArgumentOutOfRangeException>(() => char.ConvertFromUtf32(-1));
            Assert.Throws<ArgumentOutOfRangeException>(() => char.ConvertFromUtf32(int.MaxValue));
            Assert.Throws<ArgumentOutOfRangeException>(() => char.ConvertFromUtf32(int.MinValue));
        }
        ...
        {
            Assert.Throws<ArgumentException>(() => char.ConvertToUtf32("\uD800\uD800", 0)); // High, high
            Assert.Throws<ArgumentException>(() => char.ConvertToUtf32("\uD800\uD7FF", 0)); // High, non-surrogate
            Assert.Throws<ArgumentException>(() => char.ConvertToUtf32("\uD800\u0000", 0)); // High, non-surrogate
            Assert.Throws<ArgumentException>(() => char.ConvertToUtf32("\uD800\uD800", 1)); // High, high

            Assert.Throws<ArgumentException>(() => char.ConvertToUtf32("\uDC01\uD940", 0)); // Low, high
            Assert.Throws<ArgumentException>(() => char.ConvertToUtf32("\uDD00\uDE00", 0)); // Low, low
            Assert.Throws<ArgumentException>(() => char.ConvertToUtf32("\uDF01\u0000", 0)); // Low, non-surrogate
            Assert.Throws<ArgumentException>(() => char.ConvertToUtf32("\uDC01\uD940", 1)); // Low, high
            Assert.Throws<ArgumentException>(() => char.ConvertToUtf32("\uDD00\uDE00", 1)); // Low, high

            Assert.Throws<ArgumentNullException>(() => char.ConvertToUtf32(null, 0)); // String is null
            Assert.Throws<ArgumentOutOfRangeException>(() => char.ConvertToUtf32("abcde", -1)); // Index < 0
            Assert.Throws<ArgumentOutOfRangeException>(() => char.ConvertToUtf32("abcde", 5)); // Index >= string.Length
            Assert.Throws<ArgumentOutOfRangeException>(() => char.ConvertToUtf32("", 0)); // Index >= string.Length
        }
        ...
        {
            Assert.Throws<ArgumentOutOfRangeException>(() => char.ConvertToUtf32('\uD800', '\uD800')); // High, high
            Assert.Throws<ArgumentOutOfRangeException>(() => char.ConvertToUtf32('\uD800', '\uD7FF')); // High, non-surrogate
            Assert.Throws<ArgumentOutOfRangeException>(() => char.ConvertToUtf32('\uD800', '\u0000')); // High, non-surrogate

            Assert.Throws<ArgumentOutOfRangeException>(() => char.ConvertToUtf32('\uDD00', '\uDE00')); // Low, low
            Assert.Throws<ArgumentOutOfRangeException>(() => char.ConvertToUtf32('\uDC01', '\uD940')); // Low, high
            Assert.Throws<ArgumentOutOfRangeException>(() => char.ConvertToUtf32('\uDF01', '\u0000')); // Low, non-surrogate

            Assert.Throws<ArgumentOutOfRangeException>(() => char.ConvertToUtf32('\u0032', '\uD7FF')); // Non-surrogate, non-surrogate
            Assert.Throws<ArgumentOutOfRangeException>(() => char.ConvertToUtf32('\u0000', '\u0000')); // Non-surrogate, non-surrogate
        }
        ...
        {
            Assert.Throws<ArgumentNullException>(() => char.GetNumericValue(null, 0)); // String is null
            Assert.Throws<ArgumentOutOfRangeException>(() => char.GetNumericValue("abc", -1)); // Index < 0
            Assert.Throws<ArgumentOutOfRangeException>(() => char.GetNumericValue("abc", 3)); // Index >= string.Length
        }
        ...
        {
            Assert.Throws<ArgumentNullException>(() => char.IsControl(null, 0)); // String is null
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsControl("abc", -1)); // Index < 0
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsControl("abc", 3)); // Index >= string.Length
        }
        ...
        {
            Assert.Throws<ArgumentNullException>(() => char.IsDigit(null, 0)); // String is null
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsDigit("abc", -1)); // Index < 0
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsDigit("abc", 3)); // Index >= string.Length
        }
        ...
        {
            Assert.Throws<ArgumentNullException>(() => char.IsLetter(null, 0)); // String is null
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsLetter("abc", -1)); // Index < 0
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsLetter("abc", 3)); // Index >= string.Length
        }
        ...
        {
            Assert.Throws<ArgumentNullException>(() => char.IsLetterOrDigit(null, 0)); // String is null
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsLetterOrDigit("abc", -1)); // Index < 0
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsLetterOrDigit("abc", 3)); // Index >= string.Length
        }
        ...
        [Fact]
        public static void TestIsLowerChar_String_Int()
        {
            foreach (var c in GetTestChars(UnicodeCategory.LowercaseLetter))
        ...
        {
            Assert.Throws<ArgumentNullException>(() => char.IsLower(null, 0)); // String is null
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsLower("abc", -1)); // Index < 0
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsLower("abc", 3)); // Index >= string.Length
        }
        ...
        {
            Assert.Throws<ArgumentNullException>(() => char.IsNumber(null, 0)); // String is null
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsNumber("abc", -1)); // Index < 0
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsNumber("abc", 3)); // Index >= string.Length
        }
        ...
        {
            Assert.Throws<ArgumentNullException>(() => char.IsPunctuation(null, 0)); // String is null
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsPunctuation("abc", -1)); // Index < 0
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsPunctuation("abc", 3)); // Index >= string.Length
        }
        ...
        {
            Assert.Throws<ArgumentNullException>(() => char.IsSeparator(null, 0)); // String is null
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsSeparator("abc", -1)); // Index < 0
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsSeparator("abc", 3)); // Index >= string.Length
        }
        ...
        {
            Assert.Throws<ArgumentNullException>(() => char.IsLowSurrogate(null, 0)); // String is null
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsLowSurrogate("abc", -1)); // Index < 0
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsLowSurrogate("abc", 3)); // Index >= string.Length
        }
        ...
        {
            Assert.Throws<ArgumentNullException>(() => char.IsHighSurrogate(null, 0)); // String is null
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsHighSurrogate("abc", -1)); // Index < 0
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsHighSurrogate("abc", 3)); // Index >= string.Length
        }
        ...
        {
            Assert.Throws<ArgumentNullException>(() => char.IsSurrogate(null, 0)); // String is null
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsSurrogate("abc", -1)); // Index < 0
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsSurrogate("abc", 3)); // Index >= string.Length
        }
        ...
        {
            Assert.Throws<ArgumentNullException>(() => char.IsSurrogatePair(null, 0)); // String is null
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsSurrogatePair("abc", -1)); // Index < 0
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsSurrogatePair("abc", 3)); // Index >= string.Length
        }
        ...
        {
            Assert.Throws<ArgumentNullException>(() => char.IsSymbol(null, 0)); // String is null
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsSymbol("abc", -1)); // Index < 0
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsSymbol("abc", 3)); // Index >= string.Length
        }
        ...
        [Fact]
        public static void TestIsUpperChar_String_Int()
        {
            foreach (var c in GetTestChars(UnicodeCategory.UppercaseLetter))
        ...
        {
            Assert.Throws<ArgumentNullException>(() => char.IsUpper(null, 0)); // String is null
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsUpper("abc", -1)); // Index < 0
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsUpper("abc", 3)); // Index >= string.Length
        }
        ...
        {
            Assert.Throws<ArgumentNullException>(() => char.IsWhiteSpace(null, 0)); // String is null
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsWhiteSpace("abc", -1)); // Index < 0
            Assert.Throws<ArgumentOutOfRangeException>(() => char.IsWhiteSpace("abc", 3)); // Index >= string.Length
        }
