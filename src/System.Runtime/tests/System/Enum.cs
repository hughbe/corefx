...
namespace System.Runtime.Tests
{
    public static class EnumTests
    {
        ...
        [InlineData(" Red , Blue ", false, (SimpleEnum)3)]
        ...
        public static void TestParse(string value, bool ignoreCase, Enum expected)
        {
            ...
            if (!ignoreCase)
            {
                Assert.True(Enum.TryParse(value, out e));
                Assert.Equal(expected, e);

                Assert.Equal(expected, Enum.Parse(expected.GetType(), value));
            }
            
            Assert.True(Enum.TryParse(value, ignoreCase, out e));
            Assert.Equal(expected, e);
        ...
        public static void TestParse_Invalid(Type enumType, string value, bool ignoreCase, Type exceptionType)
        {
            ...
            if (!ignoreCase)
            {
                Assert.False(Enum.TryParse(value, out e));
                Assert.Equal(default(SimpleEnum), e);

                Assert.Throws(exceptionType, () => Enum.Parse(enumType, value));
            }

            Assert.False(Enum.TryParse(value, ignoreCase, out e));
            Assert.Equal(default(SimpleEnum), e);
        ...
        public static void TestHasFlag_Invalid()
        {
            ...
            Assert.Throws<ArgumentException>(() => e.HasFlag((SimpleEnum)0x3000)); // Enum is not the same type as the instance
        }

        [Fact]
        public static void TestToObject()
        {
            VerifyToObject<SByteEnum, sbyte>(42);
            VerifyToObject<SByteEnum, SByteEnum>((SByteEnum)0x42);
            VerifyToObject<UInt64Enum, ulong>(0x0123456789abcdefL);
        ...
        private static void VerifyToObject<E, T>(T value)
        {
            object oValue = value;
            object e = Enum.ToObject(typeof(E), oValue);
            Assert.Equal(e.GetType(), typeof(E));
            E expected = (E)(object)(value);
            object oExpected = expected; // Workaround for Bartok codegen bug: Calling Object methods on enum through type variable fails (due to missing box)
        ...
        [InlineData(typeof(SimpleEnum),
            new string[] { "Red", "Blue", "Green", "Green_a", "Green_b" },
